...
'
Version$ = "3.04"
' Revision .04 2018-04-12 ' Fudge for DXF output from TurboCAD v20
'       in which the text
'       box extent is miniscule in X - hence output it as
'       a multiple of the Y extent of the text box.
...
'
'Version 3.xx is rigorous version, base on true DXF specification
'
'
...
WHILE notfinished
    IF code = 0 AND value$ = "TEXT" THEN
        ...
        WT = Y + H
        WB = Y
        ' Fudge for DXF output from TurboCAD v2.0 in which the text
        ' box extent is miniscule in X - hence output it as
        ' a multiple of the Y extent of the text box.
        WR = WL + 3 * ABS(WT - WB)
